  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto flex flex-col gap-6">
        ...
        {/* Notifications */}
        {successMsg && (
          <div className="bg-brand-greenLight border border-brand-green/20 text-brand-greenText p-3 rounded-[14px] text-xs font-normal mb-2 animate-fadeIn">
            {successMsg}
          </div>
        )}
        {errorMsg && (
          <div className="bg-brand-coralLight border border-brand-coral/20 text-brand-coralText p-3 rounded-[14px] text-xs font-normal mb-2 animate-fadeIn">
            {errorMsg}
          </div>
        )}
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="text-[10px] font-normal tracking-wide uppercase px-2 py-0.5 rounded-[14px] bg-brand-coralLight text-brand-coralText border border-brand-coral/20">
                                ...
                                <span
                                  className={`text-[10px] font-normal tracking-wide uppercase px-2 py-0.5 rounded-[14px] border ${
                                    report.status === "pending"
                                      ? "bg-brand-yellowLight text-brand-yellowText border-brand-yellow/20"
                                      : report.status === "resolved"
                                      ? "bg-brand-greenLight text-brand-greenText border-brand-green/20"
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      <div className="flex justify-between items-start gap-4">
                        ...
                        <div className="flex gap-1.5 flex-shrink-0">
                          {restroom.is_hidden && (
                            <span className="bg-brand-coralLight text-brand-coralText border border-brand-coral/20 text-[10px] font-normal tracking-wide uppercase px-1.5 py-0.5 rounded-[14px]">
                              HIDDEN
                            </span>
                          )}
                          <span className="bg-neutral-100 border border-neutral-200 text-neutral-600 text-[10px] font-normal tracking-wide uppercase px-1.5 py-0.5 rounded-[14px] font-mono flex items-center gap-0.5">
                            Score: {restroom.overall_score.toFixed(1)} <Star className="w-3 h-3 fill-current text-brand-yellow" />
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-neutral-200 items-center">
                        ...
                        <motion.button
                          ...
                          className="h-8 px-3 text-xs bg-white border border-brand-coral/20 text-brand-coralText hover:bg-brand-coralLight rounded-[14px] transition-colors font-medium ml-auto"
              ...
              <div className="flex flex-col gap-4">
                <div className="relative">
                  <input
                    ...
                    className="w-full h-[48px] border-none bg-neutral-100 rounded-[14px] px-4 text-xs font-normal text-neutral-900 placeholder-neutral-400 focus:outline-none"
                  />
                </div>
                      ...
                      >
                        <div>
                          ...
                          <div className="flex gap-1.5 mt-2">
                            {prof.is_admin && (
                              <span className="bg-brand-greenLight text-brand-greenText border border-brand-green/20 text-[9px] font-normal tracking-wide uppercase px-1.5 py-0.5 rounded-[14px]">
                                ADMIN
                              </span>
                            )}
                            {prof.is_banned && (
                              <span className="bg-brand-coralLight text-brand-coralText border border-brand-coral/20 text-[9px] font-normal tracking-wide uppercase px-1.5 py-0.5 rounded-[14px]">
                                BANNED
                              </span>
                            )}
                          </div>
                          ...
                          <motion.button
                            ...
                            className={`h-8 px-3 text-xs rounded-[14px] transition-colors font-medium border ${
                              prof.is_banned
                                ...
                                : "bg-white border-brand-coral/20 text-brand-coralText hover:bg-brand-coralLight"
                            }`}
      ...
      {confirmAction && (
        <div className="fixed inset-0 z-[110] bg-neutral-900/40 backdrop-blur-[2px] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[20px] p-5 border border-brand-coral/20 shadow-card flex flex-col gap-4 text-center animate-fadeIn">
            <div className="w-10 h-10 rounded-full bg-brand-coralLight border border-brand-coral/20 flex items-center justify-center mx-auto">
            ...
            <div className="flex gap-2 mt-2">
              ...
              <motion.button
                whileTap={{ scale: 0.96 }}
                transition={{ duration: 0.08 }}
                className="flex-1 h-[52px] text-xs bg-brand-coral hover:bg-brand-coralText/20 text-white rounded-[14px] font-medium shadow-button"
                onClick={() => {
